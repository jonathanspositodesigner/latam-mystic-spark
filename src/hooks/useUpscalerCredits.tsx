  ...
  const fetchBalance = useCallback(async (attempt = 0) => {
    ...
    try {
      if (!trialCheckedRef.current) {
        ...
        await supabase.rpc('expire_landing_trial_credits', { _user_id: userId });
      }
      ...
      if (!breakdownError && breakdownData && breakdownData.length > 0) {
        ...
        setBreakdown({ total: result.total, monthly: result.monthly, lifetime: result.lifetime });
        setHasError(false);
        hasLoadedOnceRef.current = true;
      } else {
        const { data, error } = await supabase.rpc('get_upscaler_credits', { _user_id: userId });
        if (!error && data !== null) {
          setBalance(data);
  ...
  const consumeCredits = async (amount: number, description?: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc('consume_upscaler_credits', {
        _user_id: userId, _amount: amount, _description: description || 'Uso de herramienta'
      });
      if (error) return { success: false, error: error.message };
      if (!data || data.length === 0) return { success: false, error: 'Error al procesar créditos' };
      const result = data[0];
      if (!result.success) return { success: false, error: result.error_message || 'Saldo insuficiente', currentBalance: result.new_balance };
      setBalance(result.new_balance);
      fetchBalance();
      return { success: true, newBalance: result.new_balance };
    } catch (err) {
      console.error('Error consuming credits:', err);
      return { success: false, error: 'Error al consumir créditos' };
    }
  };
